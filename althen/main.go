...
func storeRefreshToken(userID int, token string, exp time.Time) error {
    _, err := db.Exec(`
...
func login(c *gin.Context) {
    ...
    _ = storeRefreshToken(user.ID, refresh, time.Now().Add(7*24*time.Hour))
...
func logout(c *gin.Context) {
    ...
    _ = revokeRefreshToken(req.RefreshToken)
...
func getAllDrinks(c *gin.Context) {
    ...
    for rows.Next() {
        var d Drink
        if err := rows.Scan(
            &d.ID, &d.SubCategory, &d.Name, &d.Price,
...
func createDrink(c *gin.Context) {
    ...
    err := db.QueryRow(`
        ...
        VALUES ($1,$2,$3,$4)
        RETURNING id, sub_category, name, price, description, created_at, updated_at
    `,
    ...
    logAudit(c.GetInt("user_id"), "create", "drinks", d.ID, gin.H{"name": d.Name}, c)
...
func updateDrink(c *gin.Context) {
    ...
    err := db.QueryRow(`
        UPDATE drinks
        SET sub_category=$1, name=$2, price=$3, description=$4
        WHERE id=$5
        RETURNING id, sub_category, name, price, description, created_at, updated_at
    `,
        d.SubCategory, d.Name, d.Price, d.Description, id,
    ).Scan(
        &d.ID, &d.SubCategory, &d.Name, &d.Price,
        &d.Description, &d.CreatedAt, &d.UpdatedAt,
    )
    ...
    logAudit(c.GetInt("user_id"), "update", "drinks", d.ID, gin.H{"name": d.Name}, c)
...
func deleteDrink(c *gin.Context) {
    ...
    result, err := db.Exec(`DELETE FROM drinks WHERE id=$1`, id)
    if err != nil {
        c.JSON(500, gin.H{"error": err.Error()})
        return
    }
    ...
    logAudit(c.GetInt("user_id"), "delete", "drinks", idInt, nil, c)

    c.JSON(200, gin.H{"message": "drink deleted"})
}
...
func getAllFoods(c *gin.Context) {
    ...
    for rows.Next() {
        var f Food
        if err := rows.Scan(
            &f.ID, &f.SubCategory, &f.Name, &f.Price,
...
func createFood(c *gin.Context) {
    ...
    logAudit(c.GetInt("user_id"), "create", "foods", f.ID, gin.H{"name": f.Name}, c)
...
func updateFood(c *gin.Context) {
    ...
    err := db.QueryRow(`
        ...
        RETURNING id, sub_category, name, price, description, created_at, updated_at
    `,
        f.SubCategory, f.Name, f.Price, f.Description, id,
    ).Scan(
        &f.ID, &f.SubCategory, &f.Name, &f.Price,
        &f.Description, &f.CreatedAt, &f.UpdatedAt,
    )
    ...
    logAudit(c.GetInt("user_id"), "update", "foods", f.ID, gin.H{"name": f.Name}, c)
...
func deleteFood(c *gin.Context) {
    ...
    result, err := db.Exec(`DELETE FROM foods WHERE id=$1`, id)
    if err != nil {
        c.JSON(500, gin.H{"error": err.Error()})
        return
    }
    ...
    idInt, _ := strconv.Atoi(id)
    logAudit(c.GetInt("user_id"), "delete", "foods", idInt, nil, c)

    c.JSON(200, gin.H{"message": "food deleted"})
}
...
func getDessert(c *gin.Context) {
    ...
    if err == sql.ErrNoRows {
        c.JSON(404, gin.H{"error": "dessert not found"})
        return
    }
    if err != nil {
        c.JSON(500, gin.H{"error": err.Error()})
...
func createDessert(c *gin.Context) {
    ...
    logAudit(c.GetInt("user_id"), "create", "desserts", d.ID, gin.H{"name": d.Name}, c)
...
func updateDessert(c *gin.Context) {
    ...
    logAudit(c.GetInt("user_id"), "update", "desserts", d.ID, gin.H{"name": d.Name}, c)
...
func deleteDessert(c *gin.Context) {
    ...
    result, err := db.Exec(`DELETE FROM desserts WHERE id=$1`, id)
    if err != nil {
        c.JSON(500, gin.H{"error": err.Error()})
        return
    }
    ...
    idInt, _ := strconv.Atoi(id)
    logAudit(c.GetInt("user_id"), "delete", "desserts", idInt, nil, c)

    c.JSON(200, gin.H{"message": "dessert deleted"})
}
...
// @version 1.0
// @description Cafe Menu API with JWT Authentication & RBAC
// @host localhost:8080
// @BasePath /api/v1
//
...
func main() {
...

    {
        ...
        api.GET("/drinks/:id", requirePermission("drinks:read"), getDrink)
        api.POST("/drinks", requirePermission("drinks:create"), createDrink)
        api.PUT("/drinks/:id", requirePermission("drinks:update"), updateDrink)
        api.DELETE("/drinks/:id", requirePermission("drinks:delete"), deleteDrink)

        //
        // FOODS
        //
        api.GET("/foods", requirePermission("foods:read"), getAllFoods)
        api.GET("/foods/:id", requirePermission("foods:read"), getFood)
        api.POST("/foods", requirePermission("foods:create"), createFood)
        api.PUT("/foods/:id", requirePermission("foods:update"), updateFood)
        api.DELETE("/foods/:id", requirePermission("foods:delete"), deleteFood)

        //
        // DESSERTS
        //
        api.GET("/desserts", requirePermission("desserts:read"), getAllDesserts)
        api.GET("/desserts/:id", requirePermission("desserts:read"), getDessert)
        api.POST("/desserts", requirePermission("desserts:create"), createDessert)
        api.PUT("/desserts/:id", requirePermission("desserts:update"), updateDessert)
        api.DELETE("/desserts/:id", requirePermission("desserts:delete"), deleteDessert)
    }
